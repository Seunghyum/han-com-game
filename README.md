...
- ReactiveComponent 클래스 정의
  - 목적 : *ViewModel* 역할. View - ViewModel을 분리해서 View의 디스플레이 로직에 관심없이 state관리와 비즈니스 로직에만 관심있게 하기위해 디자인 함. 
  - 동작방식
...
- ComponentBase 클래스 정의
  - 목적 : *View* 역할. UI 컴포넌트 모듈화를 위한 클래스. ReactiveComponent와는 반대로 디스플레이 로직에 집중.
  - 동작 방식
